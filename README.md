...
This project provides a Docker container for [MSP360 Backup](https://www.msp360.com/backup/).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of MSP360 Backup.
...
|`CBB_WEB_INTERFACE_USER`| Username to use to login to the CloudBerry Backup's web interface. | (no value) |
|`CBB_WEB_INTERFACE_PASSWORD`| Password to use to login to the CloudBerry Backup's web interface. | (no value) |
...
| 43210 | TCP | Optional | Port used to access the CloudBerry Backup's web interface via HTTP. |
| 43211 | TCP | Optional | Port used to access the CloudBerry Backup's web interface via HTTPs. |
...
[Releases]: https://github.com/jlesage/docker-cloudberry-backup/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/cloudberry-backup/tags
...
container, it would proxy all HTTP requests for `cloudberry-backup.domain.tld` to
...
server {
	...
	location /cloudberry-backup/ {
		...
		# Needed when audio support is enabled.
		location /cloudberry-backup/websockify-audio {
			proxy_pass http://docker-cloudberry-backup/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of MSP360 Backup integrated into Docker image. |
...
Any desktop notification generated by MSP360 Backup is forwarded to the
